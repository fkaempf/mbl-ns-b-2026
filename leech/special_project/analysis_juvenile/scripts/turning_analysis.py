...
# Per-video arena geometry (REAL-WORLD SCALE). Dish = 3.5 cm diameter, radius 1.75 cm.
ARENA = {
...
def get_track(video, track):
    sub = df[(df["video"] == video) & (df["track"] == track)]
    if len(sub) == 0:
        return None
...
# Each treatment is ONE video with 2 tracked leeches (L0, L1). Bars in the bar
# charts are the MEAN of a treatment's 2 leeches (4 treatments, not 8 leeches).
TREATMENTS = [
...
def treat_disp(name, low):
    return name + " (low-conf)" if low else name
...
for video, tr, label in ENTITIES:
    m = get_track(video, tr)
    if m is None:
        continue
    cmpp = ARENA[video]["cmpp"]
    cx0 = ARENA[video]["cx"]; cy0 = ARENA[video]["cy"]
    n = len(m)
    t = m["time_s"].values
    ...
    body_len_cm = float(np.nanmedian(np.hypot(hx - tx, hy - ty)))
    ...
    results[label] = {
        ...
        "dish": video, "track": tr, "low_confidence": label in LOWCONF,
        "n_frames": int(n),
        "body_len_cm": round(body_len_cm, 3),
        "cm_per_px": cmpp,
...
# One bar per TREATMENT = mean of that treatment's 2 leeches.
fig, ax = plt.subplots(figsize=(8, 5))
t_names = [treat_disp(name, low) for name, _, low in TREATMENTS]
sx = [float(np.nanmean([results[l]["mean_straightness_10s"]
                        for l in mem if l in results]))
      for _, mem, _ in TREATMENTS]
t_cols = ["#4c72b0", "#dd8452", "#55a868", "#c44e52"]
bars = ax.bar(t_names, sx, color=t_cols)
ax.set_ylabel("Mean straightness (net cm / path cm, 10 s windows)")
ax.set_title("Path tortuosity per treatment (1 = straight, low = tortuous)\n"
...
# Keep thirds grouping; within each third, one bar per TREATMENT = mean of its 2 leeches.
def treat_mean_third(metric, i, members):
    vals = [thirds_data[l][metric][i] for l in members if l in thirds_data]
    return float(np.nanmean(vals)) if vals else np.nan

fig, axs = plt.subplots(1, 2, figsize=(14, 5))
seg_names = ["early", "mid", "late"]
t_names = [treat_disp(name, low) for name, _, low in TREATMENTS]
xpos = np.arange(len(seg_names)); w = 0.2
t_cols = ["#4c72b0", "#dd8452", "#55a868", "#c44e52"]
offsets = (np.arange(len(TREATMENTS)) - (len(TREATMENTS) - 1) / 2.0) * w
for j, (name, mem, low) in enumerate(TREATMENTS):
    sp = [treat_mean_third("speed", i, mem) for i in range(len(seg_names))]
    axs[0].bar(xpos + offsets[j], sp, w, label=t_names[j], color=t_cols[j])
axs[0].set_xticks(xpos); axs[0].set_xticklabels(seg_names)
axs[0].set_ylabel("Mean speed (cm/s)")
axs[0].set_title("Speed by session third\nbars = mean of the 2 leeches per treatment")
axs[0].legend(fontsize=8)
for j, (name, mem, low) in enumerate(TREATMENTS):
    tn = [treat_mean_third("turn", i, mem) for i in range(len(seg_names))]
    axs[1].bar(xpos + offsets[j], tn, w, label=t_names[j], color=t_cols[j])
axs[1].set_xticks(xpos); axs[1].set_xticklabels(seg_names)
axs[1].set_ylabel("Turning (deg/min)")
axs[1].set_title("Turning by session third\nbars = mean of the 2 leeches per treatment")
